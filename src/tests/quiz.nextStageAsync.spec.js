...
describe("quiz.nextStageAsync()", function () {
  ...
  describe("when currently awaiting players", function () {
    ...
    it("should reject request if no players yet", async function () {
      ...
      await expect(quiz.nextStageAsync("1234")).to.eventually.be.rejectedWith(
  ...
  describe("when showing question", function () {
    ...
    it("should count how many players picked each answer", async function () {
      ...
      const instance = saveInstance.firstCall.args[0];
      expect(
        instance.questions[0].answers.find((a) => a.letter == "A").count
      ).to.be.equal(2);
      expect(
        instance.questions[0].answers.find((a) => a.letter == "B").count
      ).to.be.equal(0);
      expect(
        instance.questions[0].answers.find((a) => a.letter == "C").count
      ).to.be.equal(0);
      expect(
        instance.questions[0].answers.find((a) => a.letter == "D").count
      ).to.be.equal(1);
    });
    ...
    it("should award points to each player who gets the answer right", async function () {
      ...
      const instance = saveInstance.firstCall.args[0];
      expect(
        instance.players.find((p) => p.name == "Player 1").score
      ).to.be.equal(10 + 0);
      expect(
        instance.players.find((p) => p.name == "Player 2").score
      ).to.be.equal(50 + 10);
      expect(
        instance.players.find((p) => p.name == "Player 3").score
      ).to.be.equal(60 + 10);
    });
    ...
    it("should sort players by score (descending)", async function () {
      ...
      expect(saveInstance.calledOnce).to.be.true;
      const instance = saveInstance.firstCall.args[0];
      expect(instance.players.map((p) => p.name)).to.have.members([
        "Player 3",
        "Player 2",
        "Player 1",
      ]);
    ...
    it("should rank players by score (descending)", async function () {
      ...
      const instance = saveInstance.firstCall.args[0];
      expect(
        instance.players.find((p) => p.name == "Player 1").rank
      ).to.be.equal(3);
      expect(
        instance.players.find((p) => p.name == "Player 2").rank
      ).to.be.equal(2);
      expect(
        instance.players.find((p) => p.name == "Player 3").rank
      ).to.be.equal(1);
    });
